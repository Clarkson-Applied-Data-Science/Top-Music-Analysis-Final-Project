...
To analyze the data, I aggregated the top 100 songs of each year by averageing each individual variable. To do this, I would go through each song in the year, and for each variable, I would check to see if the value was "None." If the value was not none, I added the value to a sum total and would incriment a counter by one. At the end of the list of 100, I divided the sum total by the count to get the average of that value for the year. This was stored in a dictionary called "year_data". Once each variable had been calculated in this way, the dictionary containing the average of each variable was put into a dictionary called "all_years_data." This process would repeat for each year in the range.
...
## Visuals
In this section, I show the visuals of how music has changed over time. I also attempt to provide a breif explaination of why we see the trends that we see. I would like to note here that this explaintion is purely speculation on my part.
...
The second graph displays speechiness and instrumentalness. This graph shows us that over time there is a much stronger emphasis on lyrics and much less on intruments. This is consistent with the increased consumption of rap music.
