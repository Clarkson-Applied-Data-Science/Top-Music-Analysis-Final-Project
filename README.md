...
For reference, below are links to the Spotify API information page which shows the structure of the response as well as a brief description of how these measurements are taken and what they mean:
...
where {year} should be replaced with the four-digit year in question. This source was the deciding factor for the year range I chose. When inputting the year 1958, the length of the list on Wikipedia was 50 and after 1958, the list was 100 long. For the ease of scraping, I decided to make 1960 the first year in the range. Furthermore, having run this script in the middle of 2022, the top 100 for 2022 would not be populated yet, so 2021 was determined to be the maximum year. 
...
- There were not a lot of identifiers that would allow BeautifulSoup to quickly and accurately find the portion of the HTML that would be useful. This would cause me to use the split method instead. 
...
The program first split the HTML to only include the table of songs and artists. Then the table was split again with a delimiter that would break the table into 100 "rows." In reality, each "row" had four rows to it, where the second contained the name of the song (called the song row) and the third contained the name of the artist (called the artist row). The reason I call this a "row" however, is because it broke the table into the 100 instances of songs for that year. 

To pull the song out of the row, I split to the song row. The song name often appeared more than once in the song row. The last instance of the song name often proved to be the "cleanest" as other instances would often contain special characters to represent ampersands or apostrophes. To get at the cleanest version of the song name, I had the script count the number of times the word "title" appeared in the song row, as "title" would appear before each instance of the song name. Once the script had a count, I split to be after the last instance, and would get the song name this way. Some song rows did not have any instances of "title" so, a condition was created to split to the song name using the syntax of these rows. The song name would often still have a few errors in it, such as the string: "&amp;" appearing where there should just be an ampersand, so a replace condition was created to swap the error with the correct symbol. 
...
The artist name proved to be more varied in syntax than the song name was. For this reason, try/except was used to catch a few of the variations, the most common of which was first. If the first method failed, another method was attempted. There was one more layer below that. Each layer used a combination of splits to narrow down the artist row until only the artist name was left. Like the song, the artist name was cleaned to remove the ampersand identifier which identified a special character and replace it with the actual ampersand character. 
...
These variables were added to a dictionary called song_data. Furthermore, the song and artist were combined into one string separated by a space. This variable will be used to lookup the ID for Spotify. 
...
If the index failed (meaning that the search succeeded), "go" was set to True, clearing the program to lookup values for the song. It is worth noting here that with this try/except, "go" was only set to True in the event of a key error. In the event that the index failed, a key was created in the song_data dictionary for the ID and the first result from the response was used. I created it this way assuming that the best match for the search would be the first one listed. Other failings of the index would stop the program as usual. 

If the index to "error" succeeded, this means that the search failed, and "go" would be left as False. Ultimately, this would result in all variables other than the song and artist names being set to None.


## Spotify Data Gathering
If the program was cleared by the Go/No Go poll, it would next set out to find the data on that song using the ID. According to the Spotify API, the following url was used https://api.spotify.com/v1/audio-features/'+results[0]['id'] where "results[0]['id']" was the first ID in the list of possible ID's from the response above (explanation above). The response was a lengthy json. This time, I indexed at the stats of interest and added them to the song_data dictionary. These values are listed below. Also included is the description of these values from the Spotify API. 
- Danceability
...
Finally, I added the song's "place" to the song_data dictionary. The "place" was essentially which number it placed on the Top 100 list, so the most popular song for a given year was given a place of 1. This was done by incrementing a variable "n" for each time the program looped to look at the next song.
...
The year was incremented and the loop would repeat. Once the max year was run, the loop would end and the resulting json was dumped into a .json file saved locally. This would allow analysis of the data without needing to run the program again (the program took ~45 minutes to run on hotel wifi on my older laptop).

## The Analysis

To analyze the data, I aggregated the top 100 songs of each year by averaging each individual variable. To do this, I would go through each song in the year, and for each variable, I would check to see if the value was "None." If the value was not none, I added the value to a sum total and would increment a counter by one. At the end of the list of 100, I divided the sum total by the count to get the average of that value for the year. This was stored in a dictionary called "year_data". Once each variable had been calculated in this way, the dictionary containing the average of each variable was put into a dictionary called "all_years_data." This process would repeat for each year in the range.
...
The counter was also quite helpful in being able to identify the number of errors in finding the data. To calculate the number of errors, I subtracted the counter (which only incremented when there was a value stored in that dictionary) from 100. In this way, we could identify if there were songs that had partial data. From the table below, however, we can identify that this was case. Instead, each variable in a given year had the same number of errors. Because of this, we can speculate that these numbers instead represent the number of songs in a given year which yielded no results when looking for an ID. This could be due to an inconsistency in the HTML of the Wikipedia page that was not accounted for or Spotify not having the song on file.  It is notable that the years 1984 and 1985 had an unusually high number of errors. It is also interesting to observe that there are no errors in recent data, of which I used multiple years to model the splitting method for the scrape. 1975 is another year for which I modeled my scrape. With more time, I would analyze the differences which 1984 and 1985 had and continue to optimize this model. Summing one of these variable's errors, I find that there were 212 errors. With 100 songs per year for 62 years, this is 212/6200 data points, coming out to a 96.58% success rate. 
| year | year_danceability_err | year_energy_err | year_speechiness_err | year_instrumentalness_err | year_tempo_err | year_valence_err | year_duration_ms_err |
| ---- | --------------------- | --------------- | -------------------- | ------------------------- | -------------- | ---------------- | -------------------- |
| 1960 | 5 | 5 | 5 | 5 | 5 | 5 | 5
| 1961 | 1 | 1 | 1 | 1 | 1 | 1 | 1
| 1962 | 2 | 2 | 2 | 2 | 2 | 2 | 2
...
| 1982 | 5 | 5 | 5 | 5 | 5 | 5 | 5
| 1983 | 0 | 0 | 0 | 0 | 0 | 0 | 0
| 1984 | 28 | 28 | 28 | 28 | 28 | 28 | 28
| 1985 | 31 | 31 | 31 | 31 | 31 | 31 | 31
| 1986 | 1 | 1 | 1 | 1 | 1 | 1 | 1
| 1987 | 3 | 3 | 3 | 3 | 3 | 3 | 3
| 1988 | 4 | 4 | 4 | 4 | 4 | 4 | 4
| 1989 | 5 | 5 | 5 | 5 | 5 | 5 | 5
...
## Visuals
In this section, I show the visuals of how music has changed over time. I also attempt to provide a brief explanation of why we see the trends that we see. I would like to note here that this explanation is purely speculation on my part.

The first graph shows the average Danceability, Energy, and Valence over the 62 year period. We see a general increase in danceability and energy over time consistent with the rise of pop music. We also see a decrease in valence, meaning that the songs are trending in the direction of more negative emotions in songs.
![This is an image](/Images/top100dev.png)

The second graph displays speechiness and instrumentalness. This graph shows us that over time there is a much stronger emphasis on lyrics and much less on instruments. This is consistent with the increased consumption of rap music.
![This is an image](/Images/top100si.png)

Next, we look at the tempo over time which, for the most part, has been fairly consistent over time. It is interesting to observe the spike in tempo in 2009 and again in 2011. 
![This is an image](/Images/top100tempo.png)

Finally, the following graph displays the average duration of the top 100 songs over time. We observe that songs seem to get longer from the 1960's and peaks in the 1990's before becoming shorter once again. 
![This is an image](/Images/top100duration.png)

## Closing Thoughts
It has long interested me to know how music has changed over the years. This project has given me the opportunity to finally have my questions answered. I plan to continue analyzing this data using more of Spotify's knowledge base and cleaning up more of the errors that are shown above. The results of this test are consistent with my hypothesis and are fascinating to study.